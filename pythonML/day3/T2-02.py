...
# 곡선 공식(2차 방정식) : y(예측) = w(가중치) x X**2(특성)제곱 + w(가중치) x X**2(특성)제곱 x b(절편)
# X(특성)제곱 : 물고기 '길이'에 제곱 * 최적의 제곱 찾아서 정확도 최적화 한다
# 가중치 : 기울기
# X(특성) : 물고기 '길이'
# 가중치 : 기울기
# 절편 : y절편/편향
# 즉] X제곱 항목이 추가되면서 그래프가 U또는 곡선 모양으로 나온다. 길이가 커질수록 무게는 뻥튀기 되는 효과
import numpy as np
