...
import pandas as pd
df = pd.read_csv( './pythonML/day4/student_dataset_10000_rows.csv')
...
# [2] 모델 전수 탐색
# 다항 확장 : 특성들 간에 직선 관계가 드물다. 물고기 길이, 물고기길이 제곱, 물고기길이 세제곱 ~ ( 다항 회귀 )
...
for degree in [1,2,3,4,5]:
    poly=PolynomialFeatures(degree=degree,include_bias=False) # degree=차수
    ...
    print(f'{degree} 차수의 선형회귀 결정계수 : {r2}') # 결정계수란? 해당 모델이 예측한 결과가 얼마나 잘 성명되는지 수치화(백분율)

    optimization.append({
        'r2':r2,'model':lr,'poly':poly,'degree':degree
        ,'scaler':None,'alpha':None })
    ...
    for alpha in [0.01,0.1,1,10,100]:
        ...
        optimization.append({
        'r2':r2,'model':ridge,'poly':poly,'degree':degree
        ,'scaler':ss,'alpha':alpha })
        # 라쏘 모델
        laso=Lasso(alpha=alpha)
        laso.fit(train_scaled,train_target)
        r2=laso.score(test_scaled,test_target)
        print(f'{degree} 차수의 라쏘 강도 : {alpha}의 결정계수 : {r2}')
        optimization.append({
        'r2':r2,'model':laso,'poly':poly,'degree':degree
        ,'scaler':ss,'alpha':alpha })
...
print(f'최적의 모델 : {best_model}, 다항특성 : {best_poly}, 스케일링 : {scaler}') # 최적의 모델 : Ridge(alpha=10), 다항특성 : PolynomialFeatures(degree=3, include_bias=False), 스케일링 : StandardScaler()

# [4] 추론 함수 구현
def exam_score_predict(study_hours, attendance, sleep_hours, internet_usage, assignments_completed, previous_score): # 매개변수에 *한번에 여러 변수들을 튜플 받을 수 있다 # 키워드 매개변수
    ...
    if scaler is not None: # 만약에 scaler 사용한 모델이면
        list_scaled=scaler.transfrom() # 스케일링 적용
    best_model.predict(list_scaled)
    return 10
