...
class Service : 
    ...
    def 학습요청( self , carList ) :
        ...
        # 3) 학습모델 결정계수확인
        print( lr.score( test_input , test_target ) ) # 0.8466731821471489
        # Ridge
        ridge = Ridge()
        ridge.fit(train_input, train_target)
        print("Ridge:", ridge.score(test_input, test_target)) # Ridge: 0.8464570536896399

        # Lasso
        lasso = Lasso()
        lasso.fit(train_input, train_target)
        print("Lasso:", lasso.score(test_input, test_target)) # Lasso: 0.846381663926013
        # 4) 모델 저장
        self.model = lr 
        return True
    # 2.
