...
# 경사하강법 : 손실( 에측과 정답 오차 ) 0에 가깝게 처리하기 위한 반복 계산 ( 딥 러닝 )
# loss='log_loss'
    # 도미 확률이 51%일때 기울기(가중치)와 절편으로 수없이 조정하여 확률 100%만드는 방법( 경사 하강법 )
# loss='hinge'
    # *전체 데이터가 아닌* 도미확률이 50%, 0인 지점인 애매/아슬 한 (경계선) 자료만 가지고 확률 조정 하는 방법( 경사 하강법 )
